...
def register_callbacks(dashapp):
    ...
    # Inv STG2 Dims: PLANT_ID, YEAR_MONTH, DATE, HOUR, MINUTE

    # Update Table based on dropdowns:
    @dashapp.callback(Output('s2-rsd-table', 'data'),
                      [
                        Input('s2-rsd-table', 'pagination_settings'),
                        Input('s2-plant-id-dropdown', 'value'),
                        Input('s2-year-month-dropdown', 'value')
                      ])
    def update_rsd_check_table(ps, plant_id, year_month):

        if not rsd_check_df.empty:

            df = rsd_check_df.loc[(rsd_check_df['PLANT_ID'] == plant_id) &
                                (rsd_check_df['YEAR_MONTH'] == year_month)].copy()
    ...
    @dashapp.callback(Output('missing-recs-table', 'data'),
                      [
                          Input('s2-equip-table', 'pagination_settings'),
                          Input('s2-plant-id-dropdown', 'value'),
                          Input('s2-year-month-dropdown', 'value')
                      ])
